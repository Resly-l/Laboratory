...
Matrix Matrix::Perspective(float width, float height, float near, float far)
{
	...
	projection.r[0].x = near * 2.0f / width;
	projection.r[1].y = near * 2.0f / height;
	//projection.r[2].z = far / (far - near);
	//projection.r[3].z = -projection.r[2].z * near;

	projection.r[2].w = 1.0f;
	projection.r[3].w = 0.0f;
